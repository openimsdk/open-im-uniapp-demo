	...
	export default {
		...
		methods: {
			setPlayListener() {
				...
				innerAudioContext.onPlay(onPlayHandler)
				innerAudioContext.onPause(onPauseOrStorHandler)
				innerAudioContext.onStop(onPauseOrStorHandler)
			},
			playAudio() {
				if (this.paused) {
				...
				innerAudioContext.src = this.message.soundElem.sourceUrl
				innerAudioContext.play();
			}
		}
	}
